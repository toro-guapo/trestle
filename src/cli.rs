...
use crate::exit::{EXIT_CODE_ERROR, exit_with_error, exit_with_findings};
#[cfg(feature = "lsp")]
use crate::lsp;
#[cfg(feature = "mcp")]
use crate::mcp;
use crate::options::{Command, Options, ParseResult, print_help};
use crate::output::{ScanStats, ScanSummary, WriteContext, write};
use crate::processing::{
  RunContext, ScanContext, process_dir_with_surrounding_context, process_files,
  process_files_with_surrounding_context,
};
use crate::trestlerc::OptionsResolver;
#[cfg(feature = "validation")]
use crate::validation::SecretValidator;
use crate::watch;

#[cfg(feature = "validation")]
pub type ValidatorFactory =
  fn(&Options, Sender<AnnotatedDiagnostic>) -> Option<Arc<dyn SecretValidator>>;
...
#[cfg(feature = "validation")]
pub fn run_with_validator(validator_factory: ValidatorFactory) {
  run_inner(Some(validator_factory));
}

#[cfg(feature = "validation")]
fn make_validator(
  factory: Option<ValidatorFactory>,
  base: Option<&ScanContext>,
  sender: &Sender<AnnotatedDiagnostic>,
) -> Option<Arc<dyn SecretValidator>> {
  let factory = factory?;
  let base = base?;

  if !base.options.validate {
    return None;
  }

  factory(&base.options, sender.clone())
}

fn run_inner(
  #[cfg(feature = "validation")] validator_factory: Option<ValidatorFactory>,
) {
  ...
  #[cfg(feature = "validation")]
  if validator_factory.is_none()
    && bases.first().is_some_and(|base| base.options.validate)
  {
    exit_with_error(
      "Error: validation is only available in the trestle-net binary.",
    );
  }

  let summary = run_scans(
    &bases,
    &targets,
    #[cfg(feature = "validation")]
    validator_factory,
...
fn run_scans(
  ...
  with_writer(bases, move |sender| {
    #[cfg(feature = "validation")]
    let validator = make_validator(validator_factory, bases.first(), sender);
    ...
    for (base, target) in bases.iter().zip(targets.iter()) {
        ...
        total += scan_one(
          ...
          |ctx| {
            if let Some(file) = &target.explicit_file {
              ...
              process_dir_with_surrounding_context(ctx, &base.abs_dir);
            }
          },
        );
      }
      ...
      if base.options.deep.scans_history()
        && let Some((walker, map)) = history_state
      {
        total += scan_remaining_blobs(
          base,
          sender.clone(),
          #[cfg(feature = "validation")]
          validator.clone(),
...
fn build_history_state(
  base: &ScanContext,
) -> Option<(crate::history::HistoryWalker, crate::processing::HistoryMap)> {
  use crate::history::HistoryWalker;
  use std::collections::HashMap;
  use std::sync::Mutex as StdMutex;

  if base.options.deep == crate::options::DeepMode::Off {
    return None;
  }
...
fn scan_remaining_blobs(
  ...
  map: crate::processing::HistoryMap,
) -> usize {
  ...
  let run_context = base.make_run_context_no_cache(
    sender,
    #[cfg(feature = "validation")]
    validator,
  );
  ...
  for occurrence in occurrences {
    crate::processing::scan_history_blob(&run_context, &walker, &occurrence);
  }
...
fn run_install() {
  ...
  match crate::install::install_in(&cwd, &trestle_path) {
    Ok(changes) => {
      if changes.is_empty() {
...
fn run_uninstall() {
  ...
  match crate::install::uninstall_in(&cwd) {
    Ok(changes) => {
      if changes.is_empty() {
